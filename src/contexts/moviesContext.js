...
const reducer = (state, action) => {
  switch (action.type) {
      ...
      case "add-top":
        return {
          topRated: state.topRated.map((m) =>
            m.id === action.payload.movie.id ? { ...m, top: true } : m
          ),
          movies: [...state.movies],
          upcoming: [...state.upcoming]
        };
    ...
    case "load-topRated":
      return { topRated: action.payload.movies, movies: [...state.topRated],upcoming :[...state.upcoming] };
    case "add-review":
      return {
        movies: state.movies.map((m) =>
...
const MoviesContextProvider = (props) => {
  ...
  const addToTop = (movieId) => {
    ...
    dispatch({ type: "add-top", payload: { movie: state.topRated[index] } });
  };
